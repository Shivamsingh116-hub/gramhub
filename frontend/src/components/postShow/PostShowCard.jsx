...
const PostShowCard = ({ postData, setIsPostShow }) => {
    ...
    return (
        <AnimatePresence>
            <motion.div
                key="post-modal"
                className="fixed inset-0 z-50 flex justify-center items-center bg-gradient-to-br from-white via-blue-50 to-cyan-100"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
            ...
            >
                ...
                <motion.div
                    ...
                    onClick={(e) => e.stopPropagation()} // prevent click bubbling to background
                >
                    {/* Post Image */}
                    <motion.img
                        src={image?.url}
