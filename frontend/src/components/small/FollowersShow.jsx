...
const FollowersShow = ({ type, data, setIsComponent }) => {
  ...
  return (
    <div className="fixed  inset-0 bg-opacity-30 backdrop-blur-xs flex justify-center z-50">
      <div></div>
      <div className="bg-white rounded-lg shadow-lg my-10 p-6  w-[90%] max-w-md relative">
        <button
          onClick={() => setIsComponent(false)} // Ideally use a prop like onClose()
          ...
          {loading ? (
            ...
            <ul>
              {showData.map(user => {
                ...
                return (
                  <li role='button' onClick={async () => {
                    await navigate(`/profile-show/${user.username}`)
                    setIsComponent(false)
                  }} key={user._id} className="hover:cursor-pointer flex items-center gap-3 py-2 border-b">
                    {hasImage ? (
                      <img
                        src={user.avatarURL}
                        alt={user.username}
                        ...
                        onError={(e) => {
                          ...
                          e.target.style.display = 'none'; // Hide broken image if needed
                          // Optional: you could add state to re-render a fallback
                        }}
                      />
                    ) : (
